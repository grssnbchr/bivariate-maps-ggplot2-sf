...
In ggplot if you provide x and y coordinates directly in the `aes()` function you will get problems with anti-aliasing. For this reason we need to save all annotations that we want to make into a separate variable, that can be used as a data source in ggplot.

To find out from where to where an annotation arrow should be drawn, go for example to <https://map.geo.admin.ch> and copy the x and y coordinates of the start and the end point of the arrow.
...
map <- ggplot(municipality_prod_geo) +
  # first: draw the relief
  geom_raster(
    data = relief,
    aes(
...
annotations %>%
  pwalk(function(...) {
    ...
    # as pwalk makes a character out of everything
    current %<>%
